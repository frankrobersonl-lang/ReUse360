  ...
  private async fetchFromBeaconApi(request: BeaconReadRequest): Promise<NormalizedMeterRead[]> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      if (attempt > 0) {
        ...
        this.logger.warn(`Beacon API retry ${attempt}/${maxRetries - 1} after ${delay}ms`);
      ...
      try {
        ...
        const isTransient = isTimeout || /ECONNRESET|ECONNREFUSED|socket hang up/i.test(lastError.message);
        if (!isTransient) throw lastError;
      }
    }

    throw new Error(`Beacon API connection timeout after ${maxRetries} attempts — service did not respond within 60s. ${lastError?.message ?? ''}`);
  }
