/**
 * Local RAG seed script — no auth required.
 *
 * Usage:
 *   source apps/web/.env.local && npx tsx scripts/seed-rag.ts
 *
...
async function main() {
  ...
  if (existing > 0) {
    console.log(`RAG store already has ${existing} documents. Clearing...`)
    await db.$executeRawUnsafe('DELETE FROM rag_documents')
    console.log('Cleared.')
  }
  ...
  console.log(`\nSeeded ${SEED_DOCUMENTS.length} RAG documents.`)
}
